...
{
	JYLoadingView *loadingView =
    [[JYLoadingView alloc] initWithFrame:[aSuperview bounds]];
	if (!loadingView)
	{
		return nil;
	}
...
+ (id) createLoadingView:(CGRect)bounds withMessage:(NSString *)message{
    JYLoadingView *loadingView =
    [[JYLoadingView alloc] initWithFrame:bounds];
	if (!loadingView)
	{
		return nil;
	}
...
{
    return [JYLoadingView createLoadingView:bounds withMessage:NSLocalizedString(@"Loading...", nil)];
}
